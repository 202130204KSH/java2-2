...
#####   ◆스윙 컴포넌트 그리기
        ...
        ■스윙의 페인팅 기본  
            ...
            □모든 스윙 컴포넌트는 자신의 모양을 그리는 paintCompoenet() 메소드를 보유.  

#####   ◆그래픽 기반 GUI 프로그래밍  
  
        ■스윙 컴포넌트에 의존하지 않고 선 원 이미지 등을 이용하여 직접 하면을 구성하는 방법  
        ■그래픽 기반 GUI 프로그래밍의 학습이 필요한 이유  
            □컴포넌트의 한계를 극복하고 차트 게임 등 자유로운 콘텐츠 표현  
            □그래픽은 컴포넌트에 비해 화면 출력 속도가 빠름  
            □스윙 컴포넌트들로 모두 그래픽으로 작성되어 있어 그래픽에 대한 학습은 자바 GUI 바탕 기술을 이해하는데 도움  
...
#####   ◆그래픽의 색과 폰트  
        ...
        ■색 Color 클래스  
            □자바의 색(RGB)로 구성 각 성분은 0~255범위의 정수  
            ```java
...
#####   ◆도형그리기와 칠하기  
        ...
        ■도형그리기  
            □선,타원,사각ㄱ형,둥근모서리,사각형,원호,패 다각형 그리기  
            □선의 굵기 조절할 수 없음  
                └drawOvel(),drawRect(),drawRoundRect()  
  
        ■도형 칠하기  
            □도형을 그리고 내부를 칠하는 기능  
                └도형의 외곽선 내부를 따로 칠하는 기능 없음  
            □도형 칠하기를 위한 메소드  
                └그리기 메소드명에서 draw대신fill로 대신하면 된다  
...
#####   ◆repaint
        ...
        ■모든 컴포넌트가 가지고 있는 메소드  
        ■자바 플랫폼에게 컴포넌트 그리기를 강제로 지시하는 메소드  
        ■repaint를 호출하면 자바 플랫폼이 컴포넌트의 paintComponent()호출  
  
        ■repaint가 필요한 경우  
            □개발자가 컴포넌트를 다시 그리고자 하는 경우  
            □프로그램에서 컴포넌트의 모양이 위치를 변경하고 바로 화면에 반영시키고자 하는 경우  
            □컴포넌트가 다시 그려져야 그 때 변경된 위치에 변경된 모양으로 출력됨  
  
#####   ◆스레드와 운영체제

        ■스레드
        운영 체제에 의해 관리되는 하나의 작업 호는 태스크
        스레드와 테스크는 바꾸어 사용해도 무관

        ■멀티스레딩
        여러 스레디를 동시에 실행시키는 응용 프로그램을 작성하는 기법
...
#####   ◆멀티태스킹과 멀티스레딩  
        ...
        ■멀티태스킹 구현기술  
            □멀티프로세싱  
                └하나의응용 프로그램이 여러개의 프로세스를 생성하고 각 프로세스가 하나의 작업을 처리하는 기법  
                └각 프로세스 독립된 메모리 영역을 보유하고 실행  
                └프로세스 사이의 문맥 교환에 따른 과도한 오버헤드와 시간 소모의 문제점  
  
        ■멀티스레딩  
            □하나의 응용 프로그램이 여러 개의 스레드를 생성하고 각 스레드가 하나의 작ㅇ버을 처리하는 기법  
            □하나의 응용 프로그램에 속한 스레드는 변수 메모리 파일 오픈 테이블 등 자원으로 공유하므로 문맥 교환에 따른 오버헤드가 매주 작음  
            □현재 대부분의 운영체제가 멀티스레딩을 기본으로 하고있음  
  
#####   ◆자바 스레드와 JVM  
   
        ■자바 스레드  
            □자바 가상 기계JVM에 의해 스케쥴되는 실행 단위의 코드 블럭  
            □스레드의 생명 주기는 JVM에 의해 관리됨 JVM은 스레드 단위로 스케쥴링  
   
        ■JVM과 자바의 멀티스레딩  
            □하나의 JVM은 하나의 자바 응용 프로그램만 실행  
                └자바 응용 프로그램이 시작될 때 JVM이 함께 실행(종료도 동일)  
            □응용 플포그램은 하나 이상의 스레드로 구성 가능  
   
#####   ◆Thread 클래스를 상속받아 스레드 만글기  
...
#####   ◆TextArear로 여러줄의 입력 창 만들기  
  
        ■여러 줄의 문자열울 입력받을 수 있는 창   
            □Action 이벤트가 발생  
            □입력 가능한 문자 개수와 창의 크기는 서로 다름,응용 프로그램에서 변경할 수 있음.  
